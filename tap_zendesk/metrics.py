...
def _log_aggregate_rates(current_capture_rate, aggregate_rates):
    ...
    if not list(aggregate_rates.items()):
        ...
        for metric, value in aggregate_rates.items():
            LOGGER.info(
                "Synced average of %s %ss per %s seconds", mean(value), metric, current_capture_rate
            )
            LOGGER.info(
                "Synced minimum of %s %ss per %s seconds", min(value), metric, current_capture_rate
            )
            LOGGER.info(
                "Synced max of %s %ss per %s seconds", max(value), metric, current_capture_rate
            )
            LOGGER.info(
                "Synced total of %s %ss in %s seconds",
                sum(value),
                metric,
...
def _aggregate_rates(current_capture_rate, current_metrics_data):
    ...
    new aggregate rates"""
    LOGGER.info("Computing aggregate metrics over the previous %d seconds", current_capture_rate)
...
def _maybe_aggregate_rates(current_capture_rate, current_metrics_data):
    ...
    """
    if current_capture_rate <= _seconds_since_datetime(current_metrics_data["window_start_time"]):
...
def _capture_raw(current_metrics_data, metric):
    ...
    LOGGER.debug(
        "Raw count for metric %s is %d", metric, current_metrics_data["window_counts"][metric]
    )
