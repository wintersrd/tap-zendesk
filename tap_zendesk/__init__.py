...
API_TOKEN_CONFIG_KEYS = ["email", "api_token"]
...
SUB_STREAMS = {"tickets": ["ticket_audits", "ticket_metrics", "ticket_comments"]}
...
def validate_dependencies(selected_stream_ids):
    ...
    for parent_stream_name in SUB_STREAMS:
        ...
        for sub_stream_name in sub_stream_names:
            if (
                sub_stream_name in selected_stream_ids
                and parent_stream_name not in selected_stream_ids
            ):
...
def oauth_auth(args):
    ...
    return {"subdomain": args.config["subdomain"], "oauth_token": args.config["access_token"]}
...
def get_session(config):
    ...
    if not all(
        k in config
        for k in ["marketplace_name", "marketplace_organization_id", "marketplace_app_id"]
    ):
        return None
